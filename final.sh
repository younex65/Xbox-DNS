...
<div class="container">
  <div class="header">
    <div class="brand">
      ...
      <div>
        ...
        <div class="logged">Logged in as: <strong>{{ user }}</strong></div>
      </div>
    </div>
    <div class="controls">
  ...
  <div class="row">
    ...
    <div class="card small">
      ...
      <div class="quick">
        <form method="post" action="/update">
          <button class="btn ghost smallbtn" type="submit">Run Update (background)</button>
        </form>
        <a class="link" href="#" onclick="downloadLogs()">Download logs</a>
        <div class="hint">You will be logged out after changing credentials. Please remember your new password.</div>
      </div>
    </div>
  </div>
</div>
...
<div class="center">
  <div class="box">
    ...
    <div class="small">Default: admin / 123456</div>
  </div>
</div>
</body>
</html>
...
.box{
  background:rgba(255,255,255,0.02);padding:28px;border-radius:12px;
  box-shadow:0 10px 30px rgba(2,6,23,0.6);width:340px;
}
.input{
  width:95%;padding:10px;margin:8px 0;border-radius:8px;
  border:1px solid rgba(255,255,255,0.03);
  background:#041022;color:#dff6ff;
}
.btn{
  width:100%;padding:10px;border-radius:10px;
  border:none;background:#00c8b8;color:#012a2a;font-weight:700
}
...
modalSave.onclick = async () => {
  ...
  if (!new_user || !new_pass) {
    modalErr.textContent = "Username and password are required.";
    modalErr.style.display = "block";
    return;
  }

  if (new_pass !== new_pass_confirm) {
    modalErr.textContent = "Passwords do not match.";
    modalErr.style.display = "block";
    return;
  }
  ...
  try {
    ...
    if (j && j.success) {
      ...
      modalErr.textContent = j.error || "Unknown error";
      modalErr.style.display = "block";
    }
  } catch (e) {
    modalErr.textContent = "Network error: " + e;
    modalErr.style.display = "block";
  }
};
...
cat > webview.py <<'EOF'
#!/usr/bin/env python3
# webview.py — refactored to use templates/static files while preserving original behavior
from flask import Flask, request, redirect, url_for, session, render_template, jsonify, send_file
import subprocess, functools, os, re, html, tempfile
from werkzeug.security import generate_password_hash, check_password_hash

app = Flask(__name__, static_folder="static", template_folder="templates")
# keep same secret (if you want to change for security, change here or via env)
    ...
    try:
        if os.path.exists(CONFIG_PATH):
            with open(CONFIG_PATH, "r", encoding="utf-8") as f:
                for line in f:
                    if line.startswith("SMARTDNS_USER="):
                        user = line.strip().split("=", 1)[1]
                    elif line.startswith("SMARTDNS_PASS="):
...
def save_credentials(u, p):
    # write atomically to avoid partial file writes
    tmp_fd, tmp_path = tempfile.mkstemp(dir=os.path.dirname(CONFIG_PATH) or "/tmp")
    try:
...
# load initial credentials and hashed password
USER, PASS = load_credentials()
PASSWORD_HASH = generate_password_hash(PASS)

# ----------------- log reading & colorizing -----------------
...
def escape_and_colorize(raw):
    # similar transformations as original; produce HTML-safe output
    text = html.escape(raw)
    # timestamps
    text = re.sub(r'(\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\])', r'<span class="ts">\1</span>', text)
    # resolved lines (keeps original wording style)
...
def login():
    ...
    if request.method == "POST":
        ...
        # check both user and password hash
...
def index():
    ...
    return render_template("index.html", user=session.get("user"), logs=colored)

@app.route("/update", methods=["POST"])
@login_required
def update():
    # launch update script in background (non-blocking) as original code did
    try:
        subprocess.Popen(["/app/update-ips.sh"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception:
        # ignore errors like missing script etc.
        pass
    return redirect(url_for("index"))
...
def change_password():
    ...
    try:
        ...
        # save to config file and update in-memory values and hash
...
def download_logs():
    if not os.path.exists(LOG_FILE):
        return "No logs", 404
    try:
        # use send_file to stream the log as an attachment with a stable filename
        return send_file(LOG_FILE, as_attachment=True, download_name="xbox-smartdns-update.log")
    except TypeError:
        # older Flask versions use attachment_filename
        return send_file(LOG_FILE, as_attachment=True, attachment_filename="xbox-smartdns-update.log")
    except Exception as e:
        return f"Error preparing download: {e}", 500

# ----------------- run app -----------------
if __name__ == "__main__":
    # Keep running the same host/port as original script expects
